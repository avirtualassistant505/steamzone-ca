...
var estimateFormSchema_default = {
  version: "v1",
...
// src/quote/schema.ts
var estimateFormSchema = estimateFormSchema_default;
var fieldMap = new Map(estimateFormSchema.fields.map((field) => [field.key, field]));
...
function deletePathValue(obj, path) {
  const pieces = path.split(".");
  const out = structuredClone(obj);
  let current = out;
  for (let i = 0; i < pieces.length - 1; i += 1) {
...
function evaluateRule(rule, answers) {
  ...
  if (typeof rule.exists === "boolean") {
    const exists = value !== void 0 && value !== null && value !== "";
...
function getEstimateSchema() {
  return estimateFormSchema;
}
function getSchemaField(fieldKey) {
...
function isFieldVisible(field, answers) {
  const serviceType = answers.serviceType ?? void 0;
...
function getVisibleFieldsInOrder(answers) {
  return estimateFormSchema.fields.filter((field) => isFieldVisible(field, answers));
}
function getRequiredVisibleFieldsInOrder(answers) {
...
function withAnswerValue(answers, key, value) {
  return setPathValue(answers, key, value);
}
function withoutAnswerValue(answers, key) {
...
function buildInputUiHint(field, answers) {
  ...
  if (field.type === "integer") {
    return {
      ...
      max: typeof field.validation?.max === "number" ? field.validation.max : void 0,
...
function pruneInvisibleAnswers(answers) {
...
function buildQuestionText(field, answers) {
  const options = getFieldOptions(field, answers.serviceType ?? void 0);
...
// server/estimateAgentRuntimeEntry.ts
var estimateAgentRuntimeEntry_exports = {};
__export(estimateAgentRuntimeEntry_exports, {
  appendTranscript: () => appendTranscript,
  getSession: () => getSession,
  peekNextQuestion: () => peekNextQuestion,
  summaryState: () => summaryState,
  toolComputeQuote: () => toolComputeQuote,
  toolGetSchema: () => toolGetSchema,
  toolGetState: () => toolGetState,
  toolNextQuestion: () => toolNextQuestion,
  toolNormalizeAndValidate: () => toolNormalizeAndValidate,
  toolSetAnswer: () => toolSetAnswer,
  validateRequiredAnswers: () => validateRequiredAnswers
});

// server/estimateEngine.ts
var PRICING_STORAGE_KEY = "steamzone.pricing-config.v2";
var QUOTE_STORAGE_KEY = "steamzone.quote-history.v2";
var CARPET_SQFT_ESTIMATES = {
  under500: 425,
  "500to1000": 750,
  "1000to1500": 1250,
  "1500to2000": 1750,
...
function withRange(total, lowMultiplier, highMultiplier) {
  return {
    low: roundCurrency(total * lowMultiplier),
    high: roundCurrency(total * highMultiplier)
  };
}
function bookingModeFromConfidence(confidence) {
  if (confidence === "red") {
...
function buildConfidence(redFlags, complexityScore, yellowThreshold) {
...
function travelFee(zone, config) {
  return config.travelFees[zone];
}
function sidesFromSkylight(option) {
  if (option === "both") {
    return 2;
  }
...
function roomPackageBasePrice(rooms, config) {
  if (rooms <= 2) {
...
function computeWindowEstimate(input, config) {
  ...
  const screensCost = input.screens === "all" ? paneEstimate * config.window.addOns.screensPerPane : input.screens === "some" ? config.window.addOns.screensSome : 0;
  const tracksCost = input.tracks === "detailed" ? config.window.addOns.tracksDetailed : 0;
  const slidingCost = input.slidingRemoval === "threePanel" ? input.slidingQuantity * config.window.addOns.slidingThreePanel : input.slidingRemoval === "fivePanel" ? input.slidingQuantity * config.window.addOns.slidingFivePanel : 0;
  const patioCost = input.patioDoors === "takeApart" ? input.patioQuantity * config.window.addOns.patioTakeApart : input.patioDoors === "slideOnly" ? input.patioQuantity * config.window.addOns.patioSlideOnly : 0;
  ...
  const railingCost = input.railingGlass === "oneSide" ? config.window.addOns.railingOneSide : input.railingGlass === "twoSides" ? config.window.addOns.railingTwoSides : 0;
  const frenchCost = input.frenchPanes === "some" ? config.window.addOns.frenchSome : input.frenchPanes === "lots" ? config.window.addOns.frenchLots : 0;
  ...
  const subtotalRaw = travelFee(input.zone, config) + basePaneCost + screensCost + tracksCost + slidingCost + patioCost + skylightCost + railingCost + frenchCost + (input.sunroom ? config.window.addOns.sunroom : 0) + (input.walkoutBasement ? config.window.addOns.walkout : 0) + hardToReachCost + hardWaterCost + constructionDebrisCost;
  const subtotal = Math.max(config.window.minimumCharge, subtotalRaw);
  const durationHoursRaw = config.window.duration.baseHours + paneEstimate * config.window.duration.minutesPerPane * storeyMultiplier * scopeMultiplier / 60 + (input.tracks === "detailed" ? config.window.duration.tracksDetailedHours : 0) + input.slidingQuantity * config.window.duration.slidingRemovalEachHours + (input.patioDoors === "takeApart" ? input.patioQuantity * config.window.duration.patioTakeApartEachHours : 0) + skylightSides * input.skylightQuantity * config.window.duration.skylightEachSideHours + (input.sunroom ? config.window.duration.sunroomHours : 0) + (input.hardWaterRemoval ? config.window.duration.hardWaterHours : 0) + (input.hardToReach ? config.window.duration.hardToReachHours : 0) + (input.constructionDebris ? config.window.duration.constructionDebrisHours : 0);
  const complexityScore = (input.scope === "both" ? 2 : input.scope === "interior" ? 1 : 0) + (input.storey === "three" ? 2 : input.storey === "twoHalf" ? 1 : 0) + (input.screens === "all" ? 1 : 0) + (input.tracks === "detailed" ? 1 : 0) + (input.hardToReach ? 2 : 0) + (input.hardWaterRemoval ? 2 : 0) + (input.constructionDebris ? 3 : 0) + (input.frenchPanes === "lots" ? 2 : input.frenchPanes === "some" ? 1 : 0) + (input.skylightQuantity > 0 ? 1 : 0);
  const redFlags = [];
  ...
  if (input.storey === "three" && input.frenchPanes === "lots" && config.window.redFlags.threeStoreyFrenchLotsRequiresQuote) {
...
function computeCommercialWindowEstimate(input, config) {
  const paneEstimate = input.sizeMode === "paneCount" ? Math.max(1, input.paneCount) : Math.max(1, Math.round(input.frontageFeet * config.commercialWindow.storefront.panesPerFrontageFoot));
  const storefrontRate = input.scope === "both" ? config.commercialWindow.storefront.bothSidesPerPane : config.commercialWindow.storefront.exteriorPerPane;
  const lowRiseBaseRate = (config.commercialWindow.lowRise.perPaneMin + config.commercialWindow.lowRise.perPaneMax) / 2;
  const storeyPremium = input.storeys === "twoToThree" ? 1 + config.commercialWindow.lowRise.upperStoreyPremiumPercent / 100 : input.storeys === "fourToEight" ? 1.5 : input.storeys === "ninePlus" ? 1.9 : 1;
  let labor = 0;
  if (input.buildingType === "storefront") {
    ...
    labor = paneEstimate * lowRiseBaseRate * (input.scope === "both" ? config.commercialWindow.lowRise.bothSidesMultiplier : 1) * storeyPremium + input.glassDoors * config.commercialWindow.storefront.perGlassDoor;
  ...
  const subtotalRaw = (labor * afterHoursMultiplier + oversprayCost + hardWaterCost + travelFee(input.zone, config)) * (1 - recurringDiscount);
  const subtotal = Math.max(config.commercialWindow.minimumCharge, subtotalRaw);
  const durationHoursRaw = config.commercialWindow.duration.baseHours + paneEstimate * config.commercialWindow.duration.minutesPerPane / 60 + input.glassDoors * config.commercialWindow.duration.minutesPerDoor / 60 + (input.afterHours ? config.commercialWindow.duration.afterHoursExtraHours : 0);
  const complexityScore = (input.buildingType === "highRise" ? 4 : input.buildingType === "midRise" ? 3 : input.buildingType === "lowRise" ? 1 : 0) + (input.scope === "both" ? 1 : 0) + (input.frequency === "weekly" || input.frequency === "biweekly" ? 1 : 0) + (input.liftRequired ? 2 : 0) + (input.afterHours ? 1 : 0) + (input.overspray ? 2 : 0) + (input.hardWater ? 1 : 0);
  const redFlags = [];
...
function computeCarpetEstimate(input, config) {
  const estimatedSqft = input.estimateMode === "sqft" ? CARPET_SQFT_ESTIMATES[input.sqftBracket] : Math.max(350, input.rooms * 180);
  const basePrice = input.estimateMode === "rooms" ? roomPackageBasePrice(input.rooms, config.carpet.roomPackages) : estimatedSqft * config.carpet.baseRatePerSqft;
  ...
  const furnitureCost = input.furnitureMoving === "heavy" ? config.carpet.addOns.furnitureHeavy : input.furnitureMoving === "light" ? config.carpet.addOns.furnitureLight : 0;
  const treatmentCost = (input.advancedStainRemoval ? config.carpet.addOns.advancedStainRemoval : 0) + (input.odorElimination ? config.carpet.addOns.odorElimination : 0) + (input.petTreatment ? config.carpet.addOns.petTreatment : 0) + (input.stainProtector ? input.estimateMode === "sqft" ? estimatedSqft * config.carpet.addOns.protectorPerSqft : input.rooms * config.carpet.addOns.protectorPerRoom : 0);
  const subtotalRaw = basePrice * conditionMultiplier + stairsCost + hallwayCost + furnitureCost + treatmentCost + travelFee(input.zone, config);
  const subtotal = Math.max(config.carpet.minimumCharge, subtotalRaw);
  const durationHoursRaw = config.carpet.duration.baseHours + (input.estimateMode === "rooms" ? input.rooms * config.carpet.duration.minutesPerRoom / 60 : estimatedSqft / 100 * config.carpet.duration.minutesPer100Sqft / 60) + input.stairsSteps * config.carpet.duration.minutesPerStair / 60 + input.hallways * config.carpet.duration.minutesPerHallway / 60 + (input.advancedStainRemoval ? config.carpet.duration.stainHours : 0) + (input.odorElimination ? config.carpet.duration.odorHours : 0) + (input.stainProtector ? config.carpet.duration.protectorHours : 0);
  const complexityScore = (input.condition === "heavy" ? 3 : input.condition === "moderate" ? 1 : 0) + (input.estimateMode === "rooms" && input.rooms >= 6 ? 2 : 0) + (input.estimateMode === "sqft" && estimatedSqft > 1500 ? 2 : 0) + (input.stairsSteps >= 12 ? 1 : 0) + (input.hallways >= 2 ? 1 : 0) + (input.advancedStainRemoval ? 1 : 0) + (input.odorElimination ? 2 : 0) + (input.unusualCondition ? 3 : 0);
  const redFlags = [];
...
function computePostConstructionEstimate(input, config) {
  ...
  const interiorWindowsCost = input.interiorWindows === "none" ? 0 : config.postConstruction.addOns.interiorWindows[input.interiorWindows];
  const floorDetailingCost = input.floorDetailing === "none" ? 0 : config.postConstruction.addOns.floorDetailing[input.floorDetailing];
  const scrapingCost = input.scraping === "some" ? estimatedSqft * config.postConstruction.addOns.scrapingPerSqftSome : input.scraping === "lots" ? estimatedSqft * config.postConstruction.addOns.scrapingPerSqftLots : 0;
  ...
  const subtotalRaw = base + interiorWindowsCost + floorDetailingCost + scrapingCost + cabinetsCost + appliancesCost + specialCost + travelFee(input.zone, config);
  const subtotal = Math.max(config.postConstruction.minimumCharge, subtotalRaw);
  const durationHoursRaw = config.postConstruction.duration.baseHours + estimatedSqft / 1e3 * config.postConstruction.duration.hoursPer1000Sqft * config.postConstruction.duration.stageMultipliers[input.stage] * config.postConstruction.duration.dustMultipliers[input.dustLoad] + (input.interiorWindows === "none" ? 0 : config.postConstruction.duration.interiorWindowsHours[input.interiorWindows]) + (input.floorDetailing === "none" ? 0 : config.postConstruction.duration.floorDetailingHours[input.floorDetailing]) + (input.scraping === "some" ? estimatedSqft / 1e3 * config.postConstruction.duration.scrapingSomeHoursPer1000 : input.scraping === "lots" ? estimatedSqft / 1e3 * config.postConstruction.duration.scrapingLotsHoursPer1000 : 0) + (input.insideCabinets ? config.postConstruction.duration.cabinetsHours : 0) + (input.appliances ? config.postConstruction.duration.appliancesHours : 0) + specialHours * config.postConstruction.duration.specialDetailingHoursMultiplier;
  const complexityScore = (input.projectType === "commercial" ? 2 : 0) + (input.stage === "final" ? 1 : input.stage === "rough" ? 2 : 0) + (input.dustLoad === "heavy" ? 3 : input.dustLoad === "medium" ? 1 : 0) + (input.scraping === "lots" ? 3 : input.scraping === "some" ? 1 : 0) + (input.specialDetailing ? 2 : 0) + (input.multiTenantAccess ? 2 : 0) + (input.schedule === "tomorrow" ? 2 : 0);
  const redFlags = [];
  ...
  if (input.dustLoad === "heavy" && input.scraping === "lots" && config.postConstruction.redFlags.heavyDustWithLotsScraping) {
...
function calculateEstimate(serviceType, input, config) {
  if (serviceType === "window") {
    return computeWindowEstimate(input, config);
  }
  if (serviceType === "commercialWindow") {
    return computeCommercialWindowEstimate(input, config);
  }
  if (serviceType === "carpet") {
    return computeCarpetEstimate(input, config);
  }
  return computePostConstructionEstimate(input, config);
}
function createDefaultLeadContact() {
  return {
    fullName: "",
    address: "",
    phone: "",
...
function createDefaultWindowInput() {
  return {
    postalCode: "",
    zone: "zoneA",
    storey: "bungalow",
...
function createDefaultCommercialWindowInput() {
  return {
    postalCode: "",
    zone: "zoneA",
...
function createDefaultCarpetInput() {
  return {
    postalCode: "",
    zone: "zoneA",
    estimateMode: "rooms",
...
function createDefaultPostConstructionInput() {
  return {
    postalCode: "",
    zone: "zoneA",
...
function saveEstimateRecord(record) {
  const normalized = {
    ...record,
...
function detectZoneFromPostalCode(postalCode) {
...
function formatBookingMode(mode) {
...
async function getSupabaseAdminClient() {
  ...
  inflight = (async () => {
    try {
      ...
      const createClientCandidate = typeof mod.createClient === "function" ? mod.createClient : typeof mod.default === "function" ? mod.default : typeof mod.default?.createClient === "function" ? mod.default.createClient : void 0;
      const createClient = createClientCandidate ?? (typeof mod.SupabaseClient === "function" ? (...args) => new mod.SupabaseClient(
        ...args
      ) : void 0);
...
async function loadActivePricingConfig() {
  ...
  const { data, error } = await supabase.from("pricing_config").select("config, updated_at").eq("id", "active").maybeSingle();
...
async function saveActivePricingConfig(next) {
  const supabase = await getSupabaseAdminClient();
  if (!supabase) {
...
function readAnswer(answers, key, fallback) {
  const value = getAnswerValue(answers, key);
  return value === void 0 ? fallback : value;
}
function readContact(answers) {
  return {
...
function buildInputFromAnswers(answers) {
  const serviceType = String(readAnswer(answers, "serviceType", ""));
  if (serviceType === "window") {
...
async function computeDeterministicQuote(answersInput) {
...
var yesTokens = /* @__PURE__ */ new Set(["y", "yes", "yeah", "yep", "sure", "ok", "okay", "affirmative", "true", "1", "please do"]);
var noTokens = /* @__PURE__ */ new Set(["n", "no", "nope", "nah", "negative", "false", "0", "dont", "don't"]);
var serviceSynonyms = {
...
function extractName(input) {
  ...
  for (const pattern of patterns) {
    ...
    if (match?.[1]) {
      const parsed = match[1].split(/\band\b|\bbut\b/i)[0].trim();
      if (parsed.length >= 2) {
        return parsed;
      }
    }
...
function parseServiceType(input) {
  const normalized = normalizeTextForMatch(input);
  const allServices = Object.keys(serviceSynonyms);
...
function parseScaledNumber(token) {
  ...
  return match[2] ? base * 1e3 : base;
}
function extractInteger(input) {
...
function coerceSelect(field, input, answersSoFar) {
  const serviceType = answersSoFar.serviceType ?? void 0;
...
function conflictCheck(fieldKey, answers) {
...
function normalizeAndValidateField(fieldKey, userText, answersSoFar) {
  ...
  if (normalizedValue === null || normalizedValue === void 0 || normalizedValue === "") {
    return {
      ok: false,
      normalized_value: null,
      needs_clarification: true,
...
function validateAnswerValue(fieldKey, value, answersSoFar) {
  const field = getSchemaField(fieldKey);
  ...
  for (const field of schema.fields) {
    ...
    const visibleField = (() => {
      ...
      const allOk = allRules.every((rule) => {
        const currentValue2 = getAnswerValue(answers, rule.key);
        if (Object.prototype.hasOwnProperty.call(rule, "equals") && currentValue2 !== rule.equals) return false;
        if (Object.prototype.hasOwnProperty.call(rule, "notEquals") && currentValue2 === rule.notEquals) return false;
        if (rule.in && !rule.in.includes(currentValue2)) return false;
        if (rule.notIn && rule.notIn.includes(currentValue2)) return false;
        if (typeof rule.exists === "boolean") {
          const exists = currentValue2 !== void 0 && currentValue2 !== null && currentValue2 !== "";
          if (exists !== rule.exists) return false;
        }
        return true;
      });
      ...
      const anyOk = anyRules.length === 0 || anyRules.some((rule) => {
        const currentValue2 = getAnswerValue(answers, rule.key);
        if (Object.prototype.hasOwnProperty.call(rule, "equals") && currentValue2 === rule.equals) return true;
        if (Object.prototype.hasOwnProperty.call(rule, "notEquals") && currentValue2 !== rule.notEquals) return true;
        if (rule.in && rule.in.includes(currentValue2)) return true;
        if (rule.notIn && !rule.notIn.includes(currentValue2)) return true;
        if (typeof rule.exists === "boolean") {
          const exists = currentValue2 !== void 0 && currentValue2 !== null && currentValue2 !== "";
    ...
    const missing = currentValue === void 0 || currentValue === null || currentValue === "";
    if (missing) {
...
// server/estimateAgentSessionStore.ts
...
function isMissingTableError(message) {
  ...
  return text2.includes("estimate_sessions") && (text2.includes("does not exist") || text2.includes("relation") || text2.includes("schema cache") || text2.includes("could not find"));
}
function loadFromMemory(sessionId) {
...
async function getSession(sessionId) {
  ...
  try {
    const { data, error } = await supabase.from("estimate_sessions").select("session_id, answers, asked_keys, transcript, last_question_key, created_at, updated_at").eq("session_id", sessionId).maybeSingle();
    if (error) {
    ...
    return {
      ...
      answers: row.answers ?? {},
      asked_keys: row.asked_keys ?? [],
      transcript: row.transcript ?? [],
      last_question_key: row.last_question_key ?? null,
      created_at: row.created_at ?? nowIso2(),
      updated_at: row.updated_at ?? nowIso2()
    };
  } catch {
    return loadFromMemory(sessionId);
  }
...
// server/estimateAgentTools.ts
function nowIso3() {
  return (/* @__PURE__ */ new Date()).toISOString();
}
function computeNextQuestionFromSession(session) {
...
async function toolGetState(sessionId) {
  return getSession(sessionId);
}
async function toolNormalizeAndValidate(fieldKey, userText, answersSoFar) {
  return normalizeAndValidateField(fieldKey, userText, answersSoFar);
}
async function toolSetAnswer(sessionId, fieldKey, normalizedValue) {
  const field = getSchemaField(fieldKey);
  if (!field) {
...
async function toolNextQuestion(sessionId) {
  const session = await getSession(sessionId);
  const next = computeNextQuestionFromSession(session);
  if (next.done || !next.next_field_key) {
    return next;
  }
  const askedKeys = session.asked_keys.includes(next.next_field_key) ? session.asked_keys : [...session.asked_keys, next.next_field_key];
  await saveSession({
    ...session,
...
async function toolComputeQuote(sessionId) {
  const session = await getSession(sessionId);
...
function summaryState(session) {
  return {
    answers: session.answers,
    asked_keys: session.asked_keys,
    last_question_key: session.last_question_key,
    service_type: session.answers.serviceType ?? void 0
  };
}
async function peekNextQuestion(sessionId) {
  const session = await getSession(sessionId);
  return computeNextQuestionFromSession(session);
}

// src/estimate/core/estimateAgentCore.ts
...
var DEFAULT_USER_START = "Hi, how are you? What can I help you with today?";
var CORRECTION_CUES = /\b(actually|instead|change|replace|correction|corrections|update|correct|correcting)\b/i;
function nowIso4() {
  return (/* @__PURE__ */ new Date()).toISOString();
}
...
function fieldHasExplicitSignal(field, raw, state) {
  ...
  if (field.type === "select") {
    ...
    const options = getFieldOptions(
      field,
      typeof serviceType === "string" ? serviceType : void 0
    );
...
function collectCandidateFieldKeys(state, schema, nextKey, rawInput) {
  ...
  for (const field of schema.fields.filter((field2) => !isAnswered(field2, state.answers))) {
    if (set.has(field.key)) continue;
    if (set.size >= 6) break;
...
function summarizeStateForResponse(state, done) {
  return {
...
async function getRuntime() {
  if (!runtimeModule) {
    runtimeModule = estimateAgentRuntimeEntry_exports;
  }
...
async function appendSessionTranscript(sessionId, role, content, at = nowIso4()) {
  const runtime = await getRuntime();
  await runtime.appendTranscript(sessionId, { role, content, at });
}
async function appendTranscript2(sessionId, role, content, at = nowIso4()) {
...
function transcriptLine(inputText, channel, metadata) {
...
async function callTool(runtime, name, args, sessionId) {
  if (name === "get_schema") {
    return runtime.toolGetSchema();
  }
  if (name === "get_state") {
...
async function normalizeAndSetAnswersFromInput(runtime, sessionId, inputText, schema, state, nextHint) {
  const lower = inputText.toLowerCase();
  const candidates = collectCandidateFieldKeys(state, schema, nextHint?.done ? void 0 : nextHint?.next_field_key, lower);
  const ambiguity = [];
  const applied = [];
...
async function maybeComputeQuote(runtime, sessionId, done) {
  if (!done) {
    return null;
  }
  return runtime.toolComputeQuote(sessionId);
}
function instructionsForContext(channel) {
  const channelText = channel ? `Input channel: ${channel}.` : "Input channel: web.";
  return `${CORE_PROMPT_PREFIX}
${channelText}`;
}
async function runAgentLoop(runtime, apiKey, sessionId, inputText, channel) {
...
async function decideNextAssistantTurn(request) {
...
async function runEstimateAgentCore(request) {
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  if (!apiKey) {
    throw new Error("OPENAI_API_KEY is missing on the server. Set process.env.OPENAI_API_KEY before using this endpoint.");
  }
...
async function handlerForEstimateAgentPost(req, res, createSessionWhenMissing = false) {
  if (req.method !== "POST") {
  ...
  try {
    const response = await runEstimateAgentCore({
      session_id: hasSession ? rawSessionId : void 0,
...
export {
  ...
  appendTranscript2 as appendTranscript,
  decideNextAssistantTurn,
  getState,
  handlerForEstimateAgentPost,
  loadSchema,
