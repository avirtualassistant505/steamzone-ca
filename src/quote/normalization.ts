...
function compactSelectText(input: string): string {
  return input
    ...
    .replace(/-/g, ' to ')
    .replace(/[._]/g, ' ')
    .replace(/\bsq\s*ft\b|square\s*feet|square\s*foot/g, '')
    .replace(/\bft\b/g, '')
    .replace(/[\r\n]+/g, ' ')
    .replace(/[^a-z0-9\s]/g, ' ')
    .replace(/\s+/g, '')
...
function extractInteger(input: string): number | null {
  ...
  if (spokenNumber !== null) {
    return Math.round(spokenNumber);
  }
...
function coerceSelect(field: SchemaField, input: string, answersSoFar: Record<string, unknown>): string | null {
  ...
  if (compactInput) {
    ...
    if (rangeMatch) {
      ...
      for (const option of options) {
        const valueNormalized = compactSelectText(option.value);
        if (valueNormalized === compactSelectText(candidate)) {
          return option.value;
        }
      }
    }
