...
const AUDIO_MODE = process.env.VOICE_TEST_AUDIO_MODE?.trim() || 'system';
const AUDIO_DEVICE = process.env.VOICE_TEST_AUDIO_DEVICE?.trim() || 'BlackHole 2ch';
const HEADLESS = String(process.env.VOICE_TEST_HEADLESS || '').trim().toLowerCase() === 'true';
...
const recordedSegments = {
  initial: [5.10, 7.80],
  continueCall: [5.10, 7.80],
  serviceType: [5.10, 7.80],
  postalCode: [13.28, 14.28],
  estimateMode: [18.38, 19.72],
  rooms: [23.80, 24.80],
  condition: [29.02, 30.42],
  stairsSteps: [33.92, 35.32],
  hallways: [39.02, 40.42],
  furnitureMoving: [44.34, 45.74],
  advancedStainRemoval: [49.60, 51.00],
  odorElimination: [54.88, 56.28],
  petTreatment: [59.96, 61.36],
  stainProtector: [65.40, 66.80],
  unusualCondition: [70.62, 72.02],
  schedule: [76.20, 77.60],
  fullName: [81.34, 83.26],
  phone: [89.70, 91.10],
  address: [111.52, 115.20],
  consentToContact: [121.82, 122.30],
  marketingOptIn: [127.90, 128.38],
  confirm: [105.12, 105.60],
};
...
const promptMatchers = [
  { key: 'initial', patterns: [/how can i help/i, /what can i help/i, /what can i do for you/i, /would you like to get an estimate started/i, /specific i can answer for you/i, /continue with an estimate/i, /different question about our services/i] },
  { key: 'continueCall', patterns: [/still want to continue/i, /do you still want to continue/i, /are you still there/i, /are you still with me/i, /want me to keep going/i, /want to keep going/i, /continue the estimate on the call/i] },
...
function chooseAnswer(botLine, state) {
  ...
  for (const entry of promptMatchers) {
    if (entry.patterns.some((pattern) => pattern.test(text))) {
      if (entry.key === 'confirm') return { key: entry.key, answer: scenario.answers.confirm };
      if (entry.key === 'initial') return { key: entry.key, answer: scenario.initial };
      if (entry.key === 'continueCall') return { key: entry.key, answer: scenario.answers.continueCall };
      ...
      return { key: entry.key, answer: response };
    }
  }
  return null;
}
...
function ensureClip(text) {
  ...
  const paddedPath = path.join(tmpDir, 'clip-padded.wav');
  fs.writeFileSync(txtPath, text + '\n');
  execFileSync('say', ['-v', 'Samantha', '-r', '125', '-f', txtPath, '-o', aiffPath], { stdio: 'ignore' });
  execFileSync(
    'ffmpeg',
    ['-y', '-i', aiffPath, '-af', 'adelay=350|350,apad=pad_dur=0.45', '-ar', '48000', '-ac', '1', paddedPath],
    { stdio: 'ignore' },
  );
  fs.copyFileSync(paddedPath, wavPath);
  return wavPath;
}
...
function setAudioDevice(type, name) {
  execFileSync('SwitchAudioSource', ['-t', type, '-s', name], {
    cwd: ROOT,
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  });
...
function hasAudioDevice(name, type) {
  const list = execFileSync('SwitchAudioSource', ['-a', '-t', type], {
    cwd: ROOT,
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  });
...
function playAnswerSystem(answer) {
  const clip = ensureClip(answer);
  execFileSync('afplay', [clip], {
    cwd: ROOT,
...
function ensureRecordedSegment(fieldKey) {
  const range = recordedSegments[fieldKey];
  if (!range) return null;
  if (!fs.existsSync(RECORDED_SOURCE_WAV)) return null;
  fs.mkdirSync(RECORDED_SEGMENT_DIR, { recursive: true });
  const outPath = path.join(RECORDED_SEGMENT_DIR, `${fieldKey}.wav`);
...
function playSystemField(fieldKey, answer) {
  const recorded = ensureRecordedSegment(fieldKey);
  if (recorded) {
    execFileSync('afplay', [recorded], {
      cwd: ROOT,
      stdio: ['ignore', 'ignore', 'pipe'],
    });
    return { mode: 'recorded', path: recorded };
  }
  playAnswerSystem(answer);
...
async function main() {
  ...
  try {
    ...
    await page.locator('chat-widget').evaluate(async (el) => {
      ...
      deep.click();
      if (window.__voiceHarness?.resume) {
    ...
    while (Date.now() < deadline) {
      ...
      if (response) {
        if (AUDIO_MODE === 'system') {
          await new Promise((resolve) => setTimeout(resolve, SYSTEM_ANSWER_DELAY_MS));
          const playback = playSystemField(response.key, response.answer);
          state.audioSent.push({ prompt: botLine, field: response.key, answer: response.answer, playback, at: new Date().toISOString() });
        } else {
          const clip = ensureClip(response.answer);
          await page.evaluate(async (base64) => window.__voiceHarness.enqueue(base64), wavToBase64(clip));
          state.audioSent.push({ prompt: botLine, field: response.key, answer: response.answer, playback: { mode: 'js', path: clip }, at: new Date().toISOString() });
        }
        console.log('answered', JSON.stringify({ botLine, field: response.key, answer: response.answer, retry: state.promptRepeatCount[botLine] || 0 }));
      }
    }
    ...
    if (previousAudio && AUDIO_MODE === 'system') {
      try {
        setAudioDevice('input', previousAudio.input);
        setAudioDevice('output', previousAudio.output);
        state.restoredAudio = true;
        console.log('audio-restored', JSON.stringify(previousAudio));
      } catch (error) {
        console.error('audio-restore-failed', error instanceof Error ? error.message : String(error));
      }
    }
  }
}
